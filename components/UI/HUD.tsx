...
const VirtualControls: React.FC<{ hasImmortality: boolean, isImmortalityActive: boolean }> = ({ hasImmortality, isImmortalityActive }) => {
    const trigger = (event: string) => {
        window.dispatchEvent(new Event(event));
    };

    // Helper for button styles
    const btnBase = "backdrop-blur-md border border-white/20 shadow-lg active:scale-95 transition-transform select-none touch-none flex items-center justify-center";

    return (
        <div className="absolute inset-0 pointer-events-none z-[60] flex flex-col justify-end pb-8 px-6">
            <div className="flex justify-between items-end w-full">
                
                {/* Left/Right Controls */}
                <div className="flex gap-4 pointer-events-auto">
                    <button 
                        onPointerDown={() => trigger('control-left')}
                        className={`${btnBase} w-16 h-16 md:w-20 md:h-20 rounded-full bg-cyan-900/40 active:bg-cyan-600/60 border-cyan-500/30`}
                        aria-label="Move Left"
                    >
                        <ArrowLeft className="w-8 h-8 text-cyan-400" />
                    </button>
                    <button 
                        onPointerDown={() => trigger('control-right')}
                        className={`${btnBase} w-16 h-16 md:w-20 md:h-20 rounded-full bg-cyan-900/40 active:bg-cyan-600/60 border-cyan-500/30`}
                ...
                <div className="flex gap-4 items-end pointer-events-auto">
                    ...
                    {hasImmortality && (
                        <button 
                            onPointerDown={() => trigger('control-ability')}
                            className={`${btnBase} w-12 h-12 md:w-16 md:h-16 rounded-full mb-2 ${isImmortalityActive ? 'bg-yellow-500/50 border-yellow-400 animate-pulse' : 'bg-purple-900/40 active:bg-purple-600/60 border-purple-500/30'}`}
                            aria-label="Activate Ability"
                        >
                            <Shield className={`w-6 h-6 ${isImmortalityActive ? 'text-white' : 'text-purple-400'}`} />
                        </button>
                    )}

                    {/* Jump Button */}
                    <button 
                        onPointerDown={() => trigger('control-jump')}
                        className={`${btnBase} w-20 h-20 md:w-24 md:h-24 rounded-full bg-pink-900/40 active:bg-pink-600/60 border-pink-500/30`}
                        aria-label="Jump"
                    >
  ...
  return (
    <>
        <div className={containerClass}>
            ...
            {/* Bottom Overlay Speed Indicator - Pushed up slightly to avoid controls */}
            <div className="w-full flex justify-center pb-24 md:pb-0 md:justify-end md:items-end">
        ...
        {/* Render Virtual Controls if Playing */}
        <VirtualControls hasImmortality={hasImmortality} isImmortalityActive={isImmortalityActive} />
    </>
  );
};